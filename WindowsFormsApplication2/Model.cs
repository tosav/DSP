        ...
        public void SetData(int n, double mini, double maxi)
        {
            ...
            for (int i = 0; i < Convert.ToInt64(disp.getN()); i++)//вычисление данных для моделей и запихивание их в чарт
            {
                switch ((int) disp.get("model_k")) { 
                    ...
                    case 9: data[i] = new PointF(i, Convert.ToSingle(Convert.ToDouble(texts[0].Text) * (1 + Convert.ToDouble(texts[1].Text) * Math.Cos(2 * Math.PI * Convert.ToDouble(RussianDouble(texts[2].Text)) * i)) * Math.Cos(2 * Math.PI * Convert.ToDouble(RussianDouble(texts[3].Text)) * i + Convert.ToDouble(RussianDouble(texts[4].Text))))); break;
